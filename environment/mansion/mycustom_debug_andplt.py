...
for _ in range(num_requests):
    ...
    start_floors.append(start_floor)
    dest_floors.append(dest_floor)
    # 打印调试信息
    print(f"Request {_ + 1}: Start Floor = {start_floor}, Destination Floor = {dest_floor}")
    for floor in simulator.floors:
        print(f"Floor {floor.number}: Origins = {floor.origins:.4f}, Possibilities = {floor.possibilities:.4f}")

# 可视化
plt.figure(figsize=(12, 6))

plt.subplot(2, 1, 1)
plt.plot(start_floors, label='Start Floor')
plt.xlabel('Request Number')
plt.ylabel('Start Floor')
plt.title('Start Floor Over Time')
plt.legend()

plt.subplot(2, 1, 2)
plt.plot(dest_floors, label='Destination Floor', color='orange')
plt.xlabel('Request Number')
plt.ylabel('Destination Floor')
plt.title('Destination Floor Over Time')
plt.legend()
