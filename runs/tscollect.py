...
from liftsim.dict_fns.dict_fn import ele_dict
...
action_shape = 52
k = 10
Actornet = Net(state_shape=state_shape,action_shape = 52,hidden_sizes=[256,256,128,64,32])
...
# obss,infos = env.reset([0,1])
# obss = [flatten_state(s) for s in obss]
#
# obss= np.array(obss)
# test_act , states= Actornet(obss)
#
# dis = ele_dict(test_act)
#
# print(dis.sample())


# actions = torch.tensor(action_parts)
...
train_collector = Collector(policy=test_policy2,env=env,buffer=buffer,preprocess_fn=process_fn)
test_collector = Collector(policy=test_policy2,env=env,buffer=buffer,preprocess_fn=process_fn)

test_collector.collect(n_step=100)
# 2步的话就2个？
# trainer = ts.trainer.OnpolicyTrainer(policy=test_policy2,train_collector=train_collector,test_collector=test_collector,
#                                      step_per_epoch=100,max_epoch=10,step_per_collect=100,show_progress=True,
#                                      episode_per_test=10,repeat_per_collect=10,batch_size=1000)
#
# trainer.run()